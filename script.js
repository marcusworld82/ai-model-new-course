...
// First pass: collect all H2 lesson blocks
const rawLessons = []; // {title, body, prompts[]}
let cur = null;
...
for(let i=0;i<lines.length;i++){
  ...
  if(/^### [^#]/.test(line)){
    // sub-heading inside a lesson — render as bold label
...
// Module names defined manually to match the course
const MODULE_NAMES = [
...
// Detect module number from lesson title prefix like "Lesson 1.1", "13.1", etc.
function getModuleIndex(title){
  // e.g. "Lesson 1.1: ..." or "Lesson 13.1: ..."
  const m = title.match(/^(?:Lesson\s+)?(\d+)\./i);
  if(m) return parseInt(m[1],10)-1; // 0-based
  // Final Project and Bonus Templates
  if(/final/i.test(title)) return 13;
  if(/bonus|template/i.test(title)) return 14;
  return -1;
}

const modules = MODULE_NAMES.map((name,i)=>({
  title: name,
  id: slug(name),
  lessons: []
}));

// Bucket each lesson into its module
rawLessons.forEach(l=>{
  const mi = getModuleIndex(l.title);
  const target = mi >= 0 && mi < modules.length ? modules[mi] : null;
  if(target){
    target.lessons.push({
      title: l.title,
      id: slug(l.title),
